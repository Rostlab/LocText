...
# Sort by F-Measure:
#  * oldIFS=$IFS; IFS=$'\n'; time for f in `tail -n 1 $logsdir/*.log | grep -oP "(?<=f_measure=).*" | sort -b -k 1,1 -t " " -h | tail -n 1000`; do grep -o "f_measure=$f" $logsdir/*.log; done | tee $logsdir/loctext_1000_best_runs.log; IFS=$oldIFS
